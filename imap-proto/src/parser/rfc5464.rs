...
fn check_private_shared(i: &[u8]) -> EntryParseStage
{
    ...
    if i.starts_with(b"/shared")
    {
        return EntryParseStage::Admin(7);
    }
    return EntryParseStage::Fail(nom::Err::Error(nom::Context::Code(i, nom::ErrorKind::IsNot)));
}
...
fn check_vendor_comment(i: &[u8], l:usize) ->EntryParseStage
{
    ...
    if i[l..].starts_with(b"/vendor")
    {
        ...
        if i.len() < l+9 ||
            i[l+7] != b'/' ||
            !is_entry_component_char(i[l+8]) {
                return EntryParseStage::Fail(nom::Err::Incomplete(nom::Needed::Unknown))
            }
        return EntryParseStage::Path(l+7);
    }
    EntryParseStage::Fail(nom::Err::Error(nom::Context::Code(i, nom::ErrorKind::IsNot)))
}
...
fn check_entry_name(i: &[u8]) -> IResult<&[u8], &[u8]> {
    ...
    loop
    {
        match stage
        {
            EntryParseStage::PrivateShared(_) => {stage = check_private_shared(i);}
            EntryParseStage::Admin(l) => {stage = check_admin(i, l);}
            EntryParseStage::VendorComment(l) => {stage = check_vendor_comment(i, l);}
            EntryParseStage::Path(l) => {stage = check_path(i, l);}
            EntryParseStage::Done(l) => {return Ok((&i[l..], &i[..l]));}
            EntryParseStage::Fail(e) => {return std::result::Result::Err(e);}
        }
    }
}
...
named!(entry_list<Vec<&str>>, do_parse!(
    list: separated_list!(tag_s!(" "), map!(entry_name, slice_to_str)) >>
        (list)
));
...
named!(metadata_solicited<Response>, do_parse!(
    ...
    tail: keyval_list >>
        (Response::MailboxData(MailboxDatum::MetadataSolicited{mailbox:slice_to_str(mbox), values:tail}))
));
...
named!(metadata_unsolicited<Response>, do_parse!(
    mbox: metadata_common >>
        tail: entry_list >>
        (Response::MailboxData(MailboxDatum::MetadataUnsolicited{mailbox:slice_to_str(mbox), values:tail}))
));
...
mod tests {
    ...
    fn test_solicited_fail_2() {
        match metadata_solicited(b"METADATA \"\" (/shared/asdfg \"asdf\")\r\n") {
            Err(_) => {},
            _ => panic!("Error required when in entry name /shared is not continued with /admin, /comment or /vendor"),
        }
    }
    ...
    fn test_solicited_fail_3() {
        match metadata_solicited(b"METADATA \"\" (/private/admin \"asdf\")\r\n") {
            Err(_) => {},
            _ => panic!("Error required when in entry name /private is not continued with /comment or /vendor"),
        }
    }
    ...
    fn test_solicited_success() {
        match metadata_solicited(b"METADATA \"mbox\" (/shared/vendor/vendorname \"asdf\" /private/comment/a \"bbb\")\r\n") {
            Ok((i, Response::MailboxData(MailboxDatum::MetadataSolicited{mailbox,values}))) => {
                ...
                assert_eq!(values[0].value.as_ref().expect("None value is not expected"), "asdf");
                assert_eq!(values[1].entry, "/private/comment/a");
                assert_eq!(values[1].value.as_ref().expect("None value is not expected"), "bbb"); },
    ...
    fn test_literal_success()
    {
        ...
        match metadata_solicited(b"METADATA \"\" (/shared/vendor/vendor.coi/a {3}\r\nAAA)\r\n")
        {
            Ok((i, Response::MailboxData(MailboxDatum::MetadataSolicited{mailbox,values}))) => {
                ...
                assert_eq!(values[0].value.as_ref().expect("None value is not expected"), "AAA"); },
            Err(e) => panic!("ERR: {:?}", e),
    ...
    fn test_unsolicited_success() {
        match metadata_unsolicited(b"METADATA \"theBox\" /shared/admin/qwe /private/comment/a\r\n") {
            Ok((i, Response::MailboxData(MailboxDatum::MetadataUnsolicited{mailbox,values}))) => {
                assert_eq!(i, b"\r\n");
